...
# ## 概要
#
# 無向グラフから順に辺を「壊す」クエリと、特定の 2 頂点が連結かを
#         問うクエリが混在する。各連結性クエリに答える。
#
# ## アルゴリズム
#
# 辺の削除は Union-Find と相性が悪いので、クエリを逆順に
...
# ## 計算量
#
# O((N + M + Q) α(N))。α は Ackermann の逆関数で実用上ほぼ定数。
#
# ## 読み方の地図
#
# - 問題の型: グラフ / UnionFind / オフライン / 難易度目安: ★4
# - 要約: 辺削除クエリを時系列逆順にして Union-Find で連結性を答える典型。
